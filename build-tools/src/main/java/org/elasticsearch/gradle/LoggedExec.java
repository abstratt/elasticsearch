...
public abstract class LoggedExec extends DefaultTask implements FileSystemOperationsAware {
    ...
    public void run() {
        ...
        ExecResult execResult = execOperations.exec(execSpec -> {
            // Eager setters removed in EAP; use the property API instead
            ...
            if (getArgs().isPresent()) {
                // ExecSpec.getArgs() is now ListProperty<String> in EAP, but the task property is
                // ListProperty<Object> to accept mixed types; convert each element to String
                execSpec.getArgs().set(getArgs().get().stream().map(Object::toString).toList());
            }
